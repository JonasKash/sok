...
export const Dashboard: React.FC<DashboardProps> = ({ businessData, results }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] pb-20">
      {/* Header */}
      <header className="bg-[#111111] border-b border-slate-700 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-3 md:py-4 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="Avestra" className="h-8 w-8 object-contain" />
            <span className="font-bold text-white hidden sm:block">Avestra</span>
          </div>
          <div className="text-xs md:text-sm text-slate-400 font-medium truncate max-w-[200px] sm:max-w-none">
            Auditoria para: <span className="text-blue-400 font-bold">{businessData.name}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 md:py-10 space-y-8 md:space-y-12">
        ...
        <section className="bg-[#111111] rounded-2xl shadow-sm border border-slate-700 overflow-hidden">
             ...
             <div className="p-5 md:p-6">
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="relative pl-4 border-l-2 border-red-800/50">
                    ...
                    <div className="relative pl-4 border-l-2 border-slate-700">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide block mb-1">Passo 3: Impacto</span>
                        <div className="text-white font-semibold text-sm mb-1">
                ...
                <div className="mt-6 bg-[#1A1A1A] border border-blue-800/50 rounded-xl p-4 flex gap-4 items-start">
                    ...
                    <div>
                        <h4 className="text-sm font-bold text-blue-300 mb-1">Otimização Odontológica</h4>
        ...
        {results.sources.length > 0 && (
          <section className="pt-8 border-t border-slate-700">
             <h4 className="text-xs md:text-sm font-bold text-slate-400 mb-3 uppercase tracking-wider">Fontes de Dados</h4>
             <div className="flex flex-wrap gap-2">
                {results.sources.map((source, i) => (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="flex items-center gap-1 bg-[#1A1A1A] hover:bg-[#1F1F1F] text-slate-400 px-3 py-1 rounded-full text-[10px] md:text-xs transition-colors max-w-full truncate border border-slate-700"
                  >
